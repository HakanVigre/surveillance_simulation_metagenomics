...
n_samples_month<-50 #samples of isolates
months<-60
...
for (k in 1:series_n) {#
  ...
  for (i in 1:60) {
    pigs<-vector("numeric")
    for (j in 1:50){ # Change to number of sample
...
library("openxlsx")
write.xlsx(time_to_detect, file = "blatem_time_to_detect_50s.xlsx",rowNames = FALSE)
