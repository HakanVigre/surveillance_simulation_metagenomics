...
n_samples_month<-100 #samples of isolates
months<-60
...
for (k in 1:series_n) {#
  ...
  for (i in 1:60) {
    pigs<-vector("numeric")
    for (j in 1:100){ # Change to number of sample
...
for (k in 1:series_n){
  ...
  #5depth_ <-rtruncnorm(vec,a=167e6*0.8,b=167e6*1.2,mean=167e6,sd=(167e6-167e6*0.8)/3)
...
library("openxlsx")
write.xlsx(time_to_detect, file = "blatem_time_to_detect_100s.xlsx",rowNames = FALSE)
